  ...
  if (document.getElementById("text_or_html").selectedIndex === 0) {
    ...
    while (satisfyCondition == 1) {
      ...
      if (finalCondition < text_size - max_text_size) {
        ...
        while (document.getElementById("preeti_text").value.charAt(finalCondition) != " ") {
          finalCondition--;
        }
      } else {
      ...
      var modified_substring = document.getElementById("preeti_text").value.substring(initialCondition, finalCondition);
...
const addString = (text) => {
  ...
  let finalText = inputText.value.substring(0, startPos) + text + inputText.value.substring(endPos);
